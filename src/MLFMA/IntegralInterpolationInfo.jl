...
function truncationLCal(cubel::FT) where {FT<:Real}
    L = floor(Int, cubel*real(Params.K_0)*sqrt(3) + 2.16*NBDIGITS^(2.0/3.0)*(cubel*real(Params.K_0))^(1/3))
    return L
end
...
"""
多极子的极信息，即角谱空间采样信息
Xθs::Vector{FT}，θ方向的采样点坐标（rad单位）
Xϕs::Vector{FT}，ϕ方向的采样点坐标（rad单位）
Wθϕs::Vector{FT}，采样点权重，用于积分时使用，在MLFMA中直接乘在转移项
"""
struct PolesInfo{FT<:Real}
    Xθs::Vector{FT}
    Xϕs::Vector{FT}
    Wθϕs::Vector{FT}
...
返回值
L           ::IT， 层 截断项
levelsPoles ::Vector{PolesInfo{FT}}，从叶层到第 “2” 层的角谱空间采样信息
"""
function levelIntegralInfoCal(levelCubeEdgel::FT) where{FT<:Real}
    ...
    # 创建Poles实例保存
    Poles  =   PolesInfo{FT}(Xθs, Xϕs, Wθϕs, r̂sθsϕs)
...
θCSCT   ::SparseMatrixCSC{FT, IT} 稀疏矩阵, θ 方向插值矩阵的转置，用于左乘本层多极子矩阵，在 θ 方向反插值
ϕCSCT   ::SparseMatrixCSC{FT, IT} 稀疏矩阵, ϕ 方向插值矩阵的转置，用于左乘本层多极子矩阵，在 ϕ 方向反插值
"""
mutable struct InterpInfo{IT<:Integer, FT<:Real}
    θCSC    ::SparseMatrixCSC{FT, IT}
    ϕCSC    ::SparseMatrixCSC{FT, IT}
    θCSCT   ::SparseMatrixCSC{FT, IT}
    ϕCSCT   ::SparseMatrixCSC{FT, IT}
    InterpInfo{IT, FT}() where {IT<:Integer, FT<:Real}  =   new{IT, FT}()
end



"""带参数的构造函数"""
function InterpInfo{IT, FT}(θCSC::SparseMatrixCSC{FT, IT}, ϕCSC::SparseMatrixCSC{FT, IT}) where {IT<:Integer, FT<:Real}

    interpInfo  =   InterpInfo{IT, FT}()

    interpInfo.θCSC =   θCSC
    interpInfo.ϕCSC =   ϕCSC
    interpInfo.θCSCT    =   sparse(transpose(θCSC))
    interpInfo.ϕCSCT    =   sparse(transpose(ϕCSC))
...
pLevelPoles::PolesInfo{FT}， 父层多极子信息
tLevelPoles::PolesInfo{FT}， 本层多极子信息
"""
function interpolationCSCMatCal(pLevelPoles::PolesInfo{FT}, tLevelPoles::PolesInfo{FT}, nlocalInterp::IT) where {IT<:Integer, FT<:Real}
    ...
    # 采用两步插值:
    # 第一步在 ϕ 方向插值，将采样点数量从本层的 ntXθs*ntXϕs 插值到临时的 ntXθs*npXϕs
    ...
    # 将权重保存在具名元组并返回
    interpWθϕ   =   InterpInfo{IT, FT}(interpθCSC, interpϕCSC)
