...
function excitationVectorMFIE!(V::Vector{Complex{FT}}, source::ST, trianglesInfo::Vector{TriangleInfo{IT, FT}}, sbfType = VSBFTypes.sbfType) where{ST<:ExcitingSources, IT<:Integer, FT<:Real}
    ...
    @threads for i in 1:nt
        ...
        tri = trianglesInfo[i]
        # 三角形相关激励向量
        Vtri    =   excitationVectorMFIE(source, tri, sbfType)
        # 写入结果
        for ni in 1:3
            n = tri.inBfsID[ni]
            # 跳过半基函数
...
nbf             ::Integer，基函数数目
"""
function excitationVectorMFIE(source::ST, trianglesInfo::Vector{TriangleInfo{IT, FT}}, nbf::Integer) where{ST<:ExcitingSources, IT<:Integer, FT<:Real}

    # 预构造激励向量
    V   =   zeros(Complex{FT}, nbf)
    # 面基函数的类型
    sbfType =   VSBFTypes.sbfType
    excitationVectorMFIE!(V, source, trianglesInfo, sbfType)
...
"""
function excitationVectorMFIE(source::ST, tri::TriangleInfo{IT, FT}, ::Type{BFT}) where {ST<:ExcitingSources, IT<:Integer, FT<:Real, BFT<:RWG}
    # 复数类型
    CT  =   Complex{FT}
    # 所有的高斯求积点
    ...
    @views for mi in 1:3
        ...
        for g in 1:GQPNTri           
            ...
            # 累加结果
            Vtri[mi]   +=   (ρm ⋅ (n̂ × view(Hgs, :, g)))*TriGQInfo.weight[g]
